export default class Graph {
  ...
  addVertex(v) {
    ...
    //Método que adiciona uma aresta do grafo
    addEdge(v, w) { // v e w sao vértices 
      ...
      //Se o grafo não for direfionado, devemos criar também a aresta w -> v
    ...
    removeVertex(v) {
      ...
      else throw new Exception('ERRO: impossível excluir um vértica com arestas incidentes a ele.')
    }

    //Método que remove uma aresta do grafo
    removeEdge(v,w) { // v e w são os vértices indidentes à aresta
      //Verifica se tanto v quanto w são vértices válidos
      if(!(this.vertices.has(v) && this.vertices.has(w))) return
      
      //Exclui w da lista de adjacêmcoa de v
      this.adjList.get(v).delete(w)

      //Se o grafo não for direcionado, precisamos excluir também a aresta em sentido oposto
